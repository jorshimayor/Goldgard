...
contract HedgeReserve is Ownable2Step {
    ...
    function convertToken0ToToken1(PoolKey calldata key, uint256 amount0In, uint32 twapWindowSeconds)
        external
        returns (uint256 amount1Out)
    {
        ...
        if (key.currency1.balanceOfSelf() < amount1Out) revert InsufficientLiquidity();
        key.currency1.transfer(msg.sender, amount1Out);
    }

    function convertToken1ToToken0(PoolKey calldata key, uint256 amount1In, uint32 twapWindowSeconds)
        external
        returns (uint256 amount0Out)
    {
        ...
        if (key.currency0.balanceOfSelf() < amount0Out) revert InsufficientLiquidity();
